...
namespace App\Module\Velox\Environment\Service;
...
final readonly class EnvironmentFileService
{
    ...
    {
        if (!$this->files->exists($this->envFilePath)) {
            return [];
        }

        $content = $this->files->read($this->envFilePath);
    ...
    {
        if ($this->createBackup && $this->files->exists($this->envFilePath)) {
    ...
    public function createBackupFile(): void
    {
        ...
        if (!$this->files->copy($this->envFilePath, $backupPath)) {
    ...
    public function restoreFromBackup(): void
    {
        ...
        if (!$this->files->copy($latestBackup, $this->envFilePath)) {
